    ...
    <div className={style.budgetsContainer}>
      ...
      <div className={style.budget_grid}>
        ...
        <div className={style.budget_status}>
          <div className={style.status_cart}>
            <div className={style.status_cart_Header}>
              <h1 className={style.status_title}>
                <p></p>Entertainment
              </h1>
              <button onClick={toggleModal}>
          {edditBudget ? <img src="./images/icon-ellipsis.svg" alt="" /> :  <img src="./images/icon-ellipsis.svg" alt="" />}
              </button>
              {edditBudget && <div className={style.edit}><div className={style.editMessage} onClick={()=> setEditModal(true)} on>Edit Budget</div> <div className={style.deleteMessage} onClick={()=> setDelete(true)}>Delete Budget</div></div>}
              {addBudget &&  <Modal   setAddBudget={setAddBudget} title={'Add New Budget'} type={'number'} category={'Choose a category to set a spending budget. These categories can help you monitor spending.'} Dot={Dot} GroupedSelect={GroupedSelect} button={'Add Budget'}/>
            }
            {editModal && <Modal   setAddBudget={setEditModal} title={'Edit Budget'} type={'number'} category={'As your budgets change, feel free to update your spending limits.'} Dot={Dot} GroupedSelect={GroupedSelect} button={'Save Changes'} />}
            {deleteModal && <Modal   setAddBudget={setDelete} title={'Delete ‘Entertainment’?'} type={null} category={'Are you sure you want to delete this budget? This action cannot be reversed, and all the data inside it will be removed forever.'} Dot={null} GroupedSelect={null} button={null} delate={'Yes, Confirm Deletion'} retry={'No, Go Back'}/>}
            </div>
            <div>
              <BudgetProgress />
            </div>
